...
DANGER_THRESHOLD = 1630  # Jarak bahaya dalam mm
MIN_VALID_DISTANCE = 255  # Jarak minimum valid (mm) untuk menghindari noise
CRITICAL_DANGER_THRESHOLD = 1015  # Jarak kritis untuk override UWB (mm)
...
# Add more granular regions for smoother navigation
FRONT_LEFT_REGION = (230, 260)
FRONT_RIGHT_REGION = (280, 310)
SLIGHT_LEFT_REGION = (250, 270)
SLIGHT_RIGHT_REGION = (270, 290)
FAR_LEFT_REGION = (180, 230)
FAR_RIGHT_REGION = (310, 360)
...
WALL_FOLLOW_DISTANCE = 1500  # Target distance to maintain from wall (mm)
WALL_FOLLOW_DURATION = 5.0   # Duration to follow wall before reassessing (seconds)
...
class LidarProcessor:
    ...
    def process_scan(self, scan_msg):
        ...
        with self.lock:
            ...
            # Detect walls for potential wall-following behavior
            self._detect_walls()
    ...
    def _detect_walls(self):
        """Detect potential walls that can be followed"""
        ...
        # Look for continuous obstacles that might be walls
        for obstacle in self.obstacles:
            # Walls are typically characterized by:
            # 1. Large angular width (continuous surface)
            # 2. Relatively consistent distance
            # 3. Located to the sides
            
            # Check if obstacle might be a wall
            if obstacle['width'] > 30:  # At least 30 degrees wide
                # Calculate distance variance to check consistency
                distances = [pt[1] for pt in obstacle['points']]
                distance_std = np.std(distances) if len(distances) > 1 else 0
                
                # If distance is fairly consistent, might be a wall
                if distance_std < 300:  # Less than 300mm standard deviation
                    center = obstacle['center']
                    distance = obstacle['distance']
                    
                    # Check if it's a left wall
                    if self._is_angle_in_region(center, LEFT_REGION) or self._is_angle_in_region(center, FAR_LEFT_REGION):
                    ...
                    elif self._is_angle_in_region(center, RIGHT_REGION) or self._is_angle_in_region(center, FAR_RIGHT_REGION):
    ...
    def _analyze_obstacles(self):
        ...
        # Also check front-diagonal regions for better obstacle detection
        for angle, distance in self.scan_data.items():
            if self._is_angle_in_region(angle, FRONT_LEFT_REGION) and distance < CRITICAL_DANGER_THRESHOLD:
                self.danger_zone = True
            if self._is_angle_in_region(angle, FRONT_RIGHT_REGION) and distance < CRITICAL_DANGER_THRESHOLD:
                self.danger_zone = True
    ...
    def get_obstacle_status(self):
        ...
        with self.lock:
            ...
            status = {
                ...
                'left_wall': self.left_wall_detected if data_valid else False,
                'right_wall': self.right_wall_detected if data_valid else False,
                'wall_distance': self.wall_distance if data_valid else float('inf'),
    ...
    def get_safe_direction(self):
        ...
        safety_scores = {
            ...
            "SLIGHT_LEFT": self._calculate_region_safety(SLIGHT_LEFT_REGION),
            "SLIGHT_RIGHT": self._calculate_region_safety(SLIGHT_RIGHT_REGION),
            "FAR_LEFT": self._calculate_region_safety(FAR_LEFT_REGION),
            "FAR_RIGHT": self._calculate_region_safety(FAR_RIGHT_REGION)
        }
        ...
        if status['front']['obstacle']:
            ...
            # Try slight turns if available
            if safety_scores["SLIGHT_LEFT"] > DANGER_THRESHOLD:
                return "SLIGHT_LEFT"
            if safety_scores["SLIGHT_RIGHT"] > DANGER_THRESHOLD:
                return "SLIGHT_RIGHT"
                
            # Otherwise, choose between left and right based on safety
            if safety_scores["LEFT"] > safety_scores["RIGHT"]:
                if safety_scores["FAR_LEFT"] > safety_scores["LEFT"]:
                    return "FAR_LEFT"
                return "LEFT"
            else:
                if safety_scores["FAR_RIGHT"] > safety_scores["RIGHT"]:
                    return "FAR_RIGHT"
                return "RIGHT"
    ...
    def find_best_avoidance_path(self, target_direction):
        ...
        # Define potential paths to evaluate - more options for better navigation
        paths = [
            ...
            ("SLIGHT_LEFT", 250, self._calculate_region_safety(SLIGHT_LEFT_REGION)),
            ("SLIGHT_RIGHT", 290, self._calculate_region_safety(SLIGHT_RIGHT_REGION)),
            ("FAR_LEFT", 200, self._calculate_region_safety(FAR_LEFT_REGION)),
            ("FAR_RIGHT", 340, self._calculate_region_safety(FAR_RIGHT_REGION))
        ]
        ...
        for name, angle, safety in paths:
            ...
            # Increased weight on target alignment to be more aggressive in following target
            target_alignment_weight = 0.5  # Was 0.3, increased to be more target-focused
            score = (safety * (1.0 - target_alignment_weight)) + (angle_factor * safety * target_alignment_weight)
...
class RobotController:
    ...
    def wall_follow(self, status, side):
        """Implement wall following behavior"""
        if side == 'left':
            target_distance = WALL_FOLLOW_DISTANCE
            current_distance = status['left']['distance']
            
            if current_distance < target_distance - 500:
                # Too close to left wall, veer right
                print("Wall follow: Too close to left wall, veering right")
                # Combination of FORWARD and RIGHT
                self.move(-self.speed/2, self.speed)
                self.current_direction = "FRONT_RIGHT"
            elif current_distance > target_distance + 500:
                # Too far from left wall, veer left
                print("Wall follow: Too far from left wall, veering left")
                # 
# Too far from left wall, veer left
                print("Wall follow: Too far from left wall, veering left")
                # Combination of FORWARD and LEFT
                self.move(-self.speed, self.speed/2)
                self.current_direction = "FRONT_LEFT"
            else:
                # Good distance, continue parallel to wall
                print("Wall follow: Maintaining good distance from left wall")
                self.move(-self.speed, self.speed)
                self.current_direction = "FORWARD"
        else:  # right wall
            target_distance = WALL_FOLLOW_DISTANCE
            current_distance = status['right']['distance']
            
            if current_distance < target_distance - 500:
                # Too close to right wall, veer left
                print("Wall follow: Too close to right wall, veering left")
                # Combination of FORWARD and LEFT
                self.move(-self.speed, self.speed/2)
                self.current_direction = "FRONT_LEFT"
            elif current_distance > target_distance + 500:
                # Too far from right wall, veer right
                print("Wall follow: Too far from right wall, veering right")
                # Combination of FORWARD and RIGHT
                self.move(-self.speed/2, self.speed)
                self.current_direction = "FRONT_RIGHT"
            else:
                # Good distance, continue parallel to wall
                print("Wall follow: Maintaining good distance from right wall")
                self.move(-self.speed, self.speed)
                self.current_direction = "FORWARD"
    ...
    def handle_obstacle_avoidance(self, lidar, target_in_view=False):
        ...
        if status['critical_danger']:
            ...
            if status['right']['distance'] > status['left']['distance'] and status['right']['distance'] > DANGER_THRESHOLD:
                ...
                if status['rear']['distance'] > DANGER_THRESHOLD * 1.5:
                    ...
                    self.last_avoidance_direction = "BACKWARD"
                    self.avoidance_start_time = time.time()
                else:
        ...
        # Check if we should continue with the previous avoidance strategy for persistence
        if (time.time() - self.avoidance_start_time < self.direction_persistence_time and 
            self.last_avoidance_direction is not None):
            print(f"Continuing previous avoidance direction: {self.last_avoidance_direction}")
            
            if self.last_avoidance_direction == "LEFT":
                self.move(-self.speed, -self.speed/self.rotation_factor)
                return True
            elif self.last_avoidance_direction == "RIGHT":
                self.move(self.speed/self.rotation_factor, self.speed)
                return True
            elif self.last_avoidance_direction == "BACKWARD":
                self.move(self.speed, -self.speed)
                return True
        
        # Check for wall following opportunity
        if (status['front']['obstacle'] and 
            (status['left_wall'] or status['right_wall']) and 
            not self.wall_following):
            # Start wall following behavior
            self.wall_following = True
            self.wall_follow_side = status['wall_side']
            self.wall_follow_start_time = time.time()
            print(f"Starting wall following on {self.wall_follow_side} side")
            return self.wall_follow(status, self.wall_follow_side)
        
        # Continue wall following if active
        if self.wall_following:
            # Check if we should stop wall following
            if time.time() - self.wall_follow_start_time > WALL_FOLLOW_DURATION:
                print("Wall following timeout - switching to normal avoidance")
                self.wall_following = False
            else:
                return self.wall_follow(status, self.wall_follow_side)
        ...
        if (status['danger_zone'] or 
            ...
            # Find best avoidance path based on obstacles and target direction
            if lidar.target_direction is not None:
                safe_direction = lidar.find_best_avoidance_path(lidar.target_direction)
            else:
                safe_direction = lidar.get_safe_direction()
                
            print(f"Calculated safe direction: {safe_direction}")
            
            if safe_direction == "STOP":
                print("DANGER ZONE DETECTED - FINDING ALTERNATE PATH")
                # Instead of stopping, try to find any safe direction
                if status['left']['distance'] > status['right']['distance']:
                    safe_direction = "LEFT"
                else:
                    safe_direction = "RIGHT"
            
            # Save the direction for persistence
            self.last_avoidance_direction = safe_direction
            self.avoidance_start_time = time.time()
                    
            # Execute movement based on safe direction
            if safe_direction == "FORWARD":
                print("Obstacle avoidance: Moving FORWARD")
                # MAJU = L+,R-
                self.move(-self.speed, self.speed)
                self.current_direction = "FORWARD"
                ...
                # KIRI = L-,R-
                # Use differential speed for smoother turning
                self.move(-self.speed, -self.speed/self.rotation_factor)
                self.current_direction = "LEFT"
            ...
            elif safe_direction == "FRONT_LEFT":
                print("Obstacle avoidance: Turning FRONT_LEFT")
                # Combined movement - more forward than left
                # Mix of MAJU (L+,R-) and KIRI (L-,R-)
                self.move(-self.speed, self.speed/2)
                self.current_direction = "FRONT_LEFT"
                
            elif safe_direction == "FRONT_RIGHT":
                print("Obstacle avoidance: Turning FRONT_RIGHT")
                # Combined movement - more forward than right
                # Mix of MAJU (L+,R-) and KANAN (L+,R+)
                self.move(-self.speed/2, self.speed)
                self.current_direction = "FRONT_RIGHT"
                
            elif safe_direction == "SLIGHT_LEFT":
                print("Obstacle avoidance: Turning SLIGHT_LEFT")
                # Very gentle left turn while moving forward
                self.move(-self.speed, self.speed * 0.8)
                self.current_direction = "SLIGHT_LEFT"
                
            elif safe_direction == "SLIGHT_RIGHT":
                print("Obstacle avoidance: Turning SLIGHT_RIGHT")
                # Very gentle right turn while moving forward
                self.move(-self.speed * 0.8, self.speed)
                self.current_direction = "SLIGHT_RIGHT"
                
            elif safe_direction == "FAR_LEFT":
                print("Obstacle avoidance: Turning FAR_LEFT")
                # Sharper left turn
                self.move(-self.speed * 1.2, -self.speed)
                self.current_direction = "FAR_LEFT"
                
            elif safe_direction == "FAR_RIGHT":
                print("Obstacle avoidance: Turning FAR_RIGHT")
                # Sharper right turn
                self.move(self.speed, self.speed * 1.2)
                self.current_direction = "FAR_RIGHT"
