...
# Konfigurasi Global
SCAN_THRESHOLD = 1000  # Jarak aman dalam mm
...
TARGET_EXCLUSION_ANGLE = 20  # Increased from 30 to 40 degrees for wider target exclusion zone
...
class LidarProcessor:
    ...
    def process_scan(self, scan_msg):
        ...
        with self.lock:
            ...
            # Process scan data - downsampling data every 5 angles
            for i in range(0, len(ranges), 5):  # Ambil setiap 5 data sudut
                distance = ranges[i]
...
class FollowingRobotNode(Node):
    ...
    def __init__(self):
        ...
        lidar_qos = QoSProfile(
            reliability=QoSReliabilityPolicy.RELIABLE,  # Pastikan data diterima secara andal
            history=QoSHistoryPolicy.KEEP_LAST,
            depth=10  # Menyimpan sejumlah data terakhir untuk menghindari kehilangan data
        )
        ...
        self.sock.settimeout(0.001)  # Reduced timeout for faster response
        ...
        self.control_timer = self.create_timer(0.001, self.control_loop)  # 100Hz control loop
        ...
        self.stats_timer = self.create_timer(0.001, self.log_statistics)
