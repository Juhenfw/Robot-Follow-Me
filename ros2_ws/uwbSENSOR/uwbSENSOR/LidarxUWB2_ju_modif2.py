...
# Konfigurasi Global
SCAN_THRESHOLD = 2500  # Jarak aman dalam mm
DANGER_THRESHOLD = 700  # Jarak bahaya dalam mm
MIN_VALID_DISTANCE = 200  # Jarak minimum valid (mm) untuk menghindari noise
CRITICAL_DANGER_THRESHOLD = 400  # Jarak kritis untuk override UWB (mm)

# UWB UDP Configuration
UDP_IP = "192.168.134.128"
UDP_PORT = 5005
...
LIDAR_SKIP_FRAMES = 1    # Process setiap frame
UWB_TIMEOUT = 0.0001     # 0.1ms timeout
MAX_LOOP_TIME = 0.015    # 15ms warning threshold
...
class LidarProcessor:
    ...
    def process_scan(self, scan_msg):
        ...
        with self.lock:
            ...
            # Downsampling agresif - ambil setiap 10 data
            step_size = 10
    ...
    def _is_angle_in_region(self, angle, region):
        ...
        if isinstance(region, tuple):
            ...
            else:  # Wraparound case (e.g. 330° to 30°)
                return angle >= start or angle <= end
        elif isinstance(region, list):  # For FRONT_REGION which is a list of tuples
            for r in region:
                start, end = r
                if start <= end:
                    if start <= angle <= end:
    ...
    def _analyze_obstacles_fast(self):
        ...
        # Single pass analysis dengan pengecekan sudut yang benar
        for angle, distance in self.scan_data.items():
            ...
            # Pengecekan region yang benar
            # Front: 330-360 dan 0-30 derajat
            is_front = (330 <= angle <= 359) or (0 <= angle <= 30)
            # Right: 31-140 derajat  
            is_right = 31 <= angle <= 140
            # Left: 220-329 derajat
            is_left = 220 <= angle <= 329
    ...
    def get_obstacle_status(self):
        ...
        with self.lock:
            ...
            data_valid = scan_age < 0.5 # Reduced from 1.0 to 0.5 seconds for faster response
            status = {
                'front': {
...
class RobotController:
    ...
    def handle_obstacle_avoidance(self, lidar, target_in_view=False):
        ...
        if not status['data_valid']:
            ...
            return False
        # If we're close to the target and the target is in view, ignore obstacles except for critical danger
        ...
        if (status['danger_zone'] or 
            ...
            if safe_direction == "STOP":
                ...
                self.obstacle_avoidance_active = True
                return True
            # New condition for turning if the left or right distance is less than threshold
    ...
    def process_control(self, uwb_distances, lidar):
        ...
        # Prioritize UWB following, with obstacle avoidance only for critical cases
...
class FollowingRobotNode(Node):
    def __init__(self):
        ...
        # Timer dengan frekuensi yang dikonfigurasi
    ...
    def execute_control_decision(self):
        ...
        uwb_data_valid = uwb_data_age < 0.05
