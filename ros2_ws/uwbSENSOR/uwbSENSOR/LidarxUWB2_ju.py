...
LIDAR_SKIP_FRAMES = 2    # Process setiap 2 frame
...
class RobotController:
    ...
    def process_control(self, uwb_distances, lidar):
        ...
        target_in_view = (
            uwb_distances['A0'] < 300 and  # Within 2 meters
...
class FollowingRobotNode(Node):
    def __init__(self):
        ...
        # Configure ROS2 QoS dengan buffer size yang dioptimasi
        lidar_qos = QoSProfile(
            reliability=QoSReliabilityPolicy.BEST_EFFORT,  # Ubah dari RELIABLE ke BEST_EFFORT
            history=QoSHistoryPolicy.KEEP_LAST,
            depth=LIDAR_BUFFER_SIZE  # Gunakan konfigurasi buffer
        )
        
        # Create LiDAR subscription dengan callback yang dioptimasi
        self.scan_subscription = self.create_subscription(
            LaserScan,
            '/scan',
            self.scan_callback_optimized,  # Gunakan callback yang dioptimasi
