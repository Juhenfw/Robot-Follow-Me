...
def main_uwb_transmitter():
    ...
    UDP_IP = "192.168.112.128"  # IP Raspberry Pi 2
    UDP_PORT = 5005           # Port untuk komunikasi
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    
    # Inisialisasi jendela average filter dengan panjang 5 untuk masing-masing nilai
    window_size = 5
    average_windows = {
                ...
                if line.startswith("$KT0"):
                    try:
                        ...
                        if len(parts) >= 4:
                            ...
                            # Update masing-masing jendela average filter
                            for key in raw_values:
                                average_windows[key].append(raw_values[key])
                            
                            # Hitung rata-rata (average) untuk setiap jendela
                            filtered_values = {}
                            for key in average_windows:
                                if len(average_windows[key]) > 0:
                                    filtered_values[key] = sum(average_windows[key]) / len(average_windows[key])
                                else:
                            ...
                            print(f"Data terkirim (Tag 0 dengan Average Filter): {uwb_data}")
