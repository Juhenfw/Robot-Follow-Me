...
# Definisi sudut untuk deteksi rintangan (updated to match the new setup)
FRONT_REGION = [(330, 360), (0, 30)]  # FRONT: 330° to 360° and 0° to 30°
RIGHT_REGION = (31, 140)  # RIGHT: 31° to 140°
LEFT_REGION  = (220, 329)  # LEFT: 220° to 329°
...
class UWBTracker:
    ...
    def estimate_target_direction(self, distances):
      ...
      diff = A2 - A1
      if abs(diff) < 20:  # Target is roughly straight ahead (within FRONT_REGION)
          target_direction = 0  # Front is considered 0 degrees (modified)
      elif diff < 0:  # A2 < A1, target is more to the right (within RIGHT_REGION)
          angle_offset = min(45, abs(diff) * 1.5)  # Scale factor to tune sensitivity
          target_direction = 90 + angle_offset  # Right of front
      else:  # A1 < A2, target is more to the left (within LEFT_REGION)
          angle_offset = min(45, abs(diff) * 1.5)  # Scale factor to tune sensitivity
          target_direction = 270 - angle_offset  # Left of front
      
      # Adjusting direction based on defined regions (modification)
      if target_direction >= 360:
          target_direction -= 360  # Ensure the direction is within 0-360 degrees
...
class LidarProcessor:
    ...
    def _analyze_obstacles_fast(self):
        ...
        front_angles = set(range(330, 360)) | set(range(0, 30))  # FRONT REGION
        right_angles = set(range(31, 141))  # RIGHT REGION
        left_angles = set(range(220, 330))  # LEFT REGION
